...
% page on the end of the eps file.
%
...
function eps2pdf(source, dest, crop, append, gray)
% Set append option
if nargin < 4
    append = false;
end
...
if nargin < 5 || ~isequal(gray, 0)
    gray = ' -sColorConversionStrategy=Gray -dProcessColorModel=/DeviceGray';
else
    gray = '';
end
% Construct the options string for ghostscript
options = [crop '-q -dNOPAUSE -dBATCH -sDEVICE=pdfwrite -dPDFSETTINGS=/prepress' gray ' -sOutputFile="' dest '"'];
